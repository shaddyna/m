// app/api/clean/yesterday/count/route.ts
import { NextRequest, NextResponse } from 'next/server';
import dbConnect from '@/lib/dbConnect';
import { withCors } from '@/lib/cors';
import { getRecordsCount, getYesterdayDateRange } from '@/lib/clean/cleanService';
  ...
  return withCors(request, async () => {
    try {
      ...
      return NextResponse.json(
        { success: false, error: err.message, message: "Server error getting yesterday's record count" },
        { status: 500 }
      );
    }
  });

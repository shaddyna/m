// app/api/clean/this-week/count/route.ts
import { NextRequest, NextResponse } from 'next/server';
import dbConnect from '@/lib/dbConnect';
import { withCors } from '@/lib/cors';
import { getRecordsCount, getThisWeekDateRange } from '@/lib/clean/cleanService';
  ...
  return withCors(request, async () => {
    try {
      ...
      return NextResponse.json(
        { success: false, error: err.message, message: "Server error getting this week's record count" },
        { status: 500 }
      );
    }
  });
